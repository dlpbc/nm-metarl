...
@register_env('point-robot-v0')
class PointEnvV0(Env):
    """
    point robot on a 2-D plane with position control
    tasks (aka goals) are positions on the plane between
    -0.5 to 0.5
    ...
    def __init__(self, randomize_tasks=True, n_tasks=2):
        ...
        goals= [[np.random.uniform(-0.5, 0.5), np.random.uniform(-0.5, 0.5)] for _ in range(n_tasks)]
        self.tasks = self.goals = goals
