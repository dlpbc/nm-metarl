...
from rlkit.torch.nmlinear import NMLinear
...
class NMMlp(PyTorchModule):
    def __init__(
        ...
        for i, next_size in enumerate(hidden_sizes):
            fc = NMLinear(in_size, next_size, nm_size)
            in_size = next_size
            hidden_init(fc.std.weight)
            hidden_init(fc.in_nm.weight)
            hidden_init(fc.out_nm.weight)
            fc.std.bias.data.fill_(b_init_value)
            fc.in_nm.bias.data.fill_(b_init_value)
            fc.out_nm.bias.data.fill_(b_init_value)
            self.__setattr__("fc{}".format(i), fc)
            self.fcs.append(fc)
