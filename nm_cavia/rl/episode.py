...
class BatchEpisodesAnalysis(BatchEpisodes):
    def __init__(self, batch_size, gamma=0.95, device='cpu'):
        super(BatchEpisodesAnalysis, self).__init__(batch_size, gamma, device)
        self._activations_list = [[] for _ in range(batch_size)]
        self._activations = None
        # for metaworld envs
        self._success_metric_list = [[] for _ in range(batch_size)]
        self._success_metric = None

    @property
    def activations(self):
        if self._activations is None:
            activation_shape = self._activations_list[0][0].shape
            activations = np.zeros((len(self), self.batch_size)
                                    + activation_shape, dtype=np.float32)
            for i in range(self.batch_size):
                length = len(self._activations_list[i])
                activations[:length, i] = np.stack(self._activations_list[i], axis=0)
            #self._activations = torch.from_numpy(activations).to(self.device)
            self._activations = activations
        return self._activations

    @property
    def success_metric(self):
        if self._success_metric is None:
            success_metric = np.zeros((len(self), self.batch_size), dtype=np.float32)
            for i in range(self.batch_size):
                length = len(self._success_metric_list[i])
                success_metric[:length, i] = np.stack(self._success_metric_list[i], axis=0)
            #self._success_metric = torch.from_numpy(success_metric).to(self.device)
            self._success_metric = success_metric
        return self._success_metric

    def append(self, observations, activations, actions, rewards, batch_ids):
        for observation, activation, action, reward, batch_id in zip(
                observations, activations, actions, rewards, batch_ids):
            if batch_id is None:
                continue
            self._observations_list[batch_id].append(observation.astype(np.float32))
            self._activations_list[batch_id].append(activation.astype(np.float32))
            self._actions_list[batch_id].append(action.astype(np.float32))
            self._rewards_list[batch_id].append(reward.astype(np.float32))

    def append(self, observations, activations, actions, rewards, batch_ids, success_metrics=None):
        if success_metrics is None:
            ...
            for observation, activation, action, reward, batch_id, success_metric in zip(
                    observations, activations, actions, rewards, batch_ids, success_metrics):
                if batch_id is None:
                    continue
                self._observations_list[batch_id].append(observation.astype(np.float32))
                self._activations_list[batch_id].append(activation.astype(np.float32))
                self._actions_list[batch_id].append(action.astype(np.float32))
                self._rewards_list[batch_id].append(reward.astype(np.float32))
                self._success_metric_list[batch_id].append(success_metric.astype(np.float32))
